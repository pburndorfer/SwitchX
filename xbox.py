...
class Joystick:
    ...
    Usage:
        joy = xbox.Joystick()
    """
    def __init__(self,refreshRate = 90):
        self.proc = subprocess.Popen(['xboxdrv','--device-by-id','045e:0719','--type','xbox360-wireless','-l','6','--no-uinput','--detach-kernel-driver'], stdout=subprocess.PIPE, bufsize=0)
    ...
    def axisScale(self,raw,deadzone=230):
        raw=raw+32769 #(range : 1 to 65536 )
        NewValue = (((raw - 1) * (0xfff - 0)) / (65536 - 1)) + 0
        if 1500 < NewValue < 2500:
            return 0x7ff
        else:
            return NewValue
